...
def aci_ae_constructor(critic, ae, i_ae, advweight, dims, visualisation_dir):
    ...
    def loss_aci_ae(x):
        ...
        x1 = x[1]
        x2 = x[2]
        loss_ae_critic = tf.reduce_mean(tf.square(x2))# tf.reduce_mean()函数是TensorFlow中用于计算张量沿指定维度上元素的均值的函数。
...
class DynAE:
    ...
    def compile_aci_ae(self, optimizer='sgd'):
        #
        self.aci_ae.compile(optimizer=optimizer, loss=total_loss)# compile()函数是用于编译Keras模型的方法。
        # 在编译过程中，你可以指定模型的优化器（optimizer）、损失函数（loss function）和评估指标（metrics）等。
    ...
    def train_aci_ae(self, x, y=None, maxiter=120e3, batch_size=256, validate_interval=2800, save_interval=2800, save_dir=PATH_RESULT, verbose=1, aug_train=True):
        ...
        for ite in range(int(maxiter)):
            ...
            loss_aci_ae = loss_aci_ae + loss1# loss1之和
            loss_disc = loss_disc + loss2# loss2之和
    ...
    def train_dynAE(self, x, y=None, kappa=3, n_clusters=10, maxiter=1e5, batch_size=256, tol=1e-2, validate_interval=140, show_interval=None, save_interval=2800, save_dir=PATH_RESULT, aug_train=True):
        ...
        for ite in range(int(maxiter)):
            ...
            if ite % save_interval == 0:
                ...
                # z_2d  = TSNE(n_components=2, random_state=35).fit_transform(z)
                # centers_2d= TSNE(n_components=2, random_state=35).fit_transform(centers_emb)
                pca = PCA(n_components=2).fit(z)
                z_2d = pca.transform(z)
                centers_2d = pca.transform(centers_emb)

                # save states for visualization
                print("___________________________________________保存画图数据——————————————————————————————————————")
                ...
                scio.savemat(self.visualisation_dir + '/embeddings/' + self.dataset + '/vis_' + str(ite) + '.mat', {"z":z,'z_2d': z_2d, 'centers_2d': centers_2d, 'y_pred': y1_pred,"y_true":y})
        ...
        scio.savemat(self.visualisation_dir + '/embeddings/' + self.dataset + '/vis' + '.mat',{"z":z,'z_2d': z_2d, 'centers_2d': centers_2d, 'y_pred': y1_pred,"y_true":y})
        print('Clustering time: %ds' % (time() - t1))
        print('End clustering:', '-' * 60)
        scio.savemat(self.visualisation_dir + '/embeddings/' + self.dataset + '/vis' + '.mat',
                     {"z": z, 'z_2d': z_2d, 'centers_2d': centers_2d, 'y_pred': y1_pred, "y_true": y})
        return y_pred
    ...
    def compute_acc_and_nmi(self, x, y):
        ...
        y_pred = km.fit_predict(features)
        print(' '*8 + '|==>  acc: %.4f,  nmi: %.4f, ari: %.4f <==|'% (metrics.acc(y, y_pred), metrics.nmi(y, y_pred), metrics.ari(y, y_pred)))
